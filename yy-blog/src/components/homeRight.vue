...
<template>
  <div class="rightlistBox">
    <section class="rs4" style=" ">
      <h2 class="ui label">热门文章</h2>
      <ul>
        <li  style="height: 25px;" v-for="(item, index) in browseList" :key="'browseList' + index">
                    <span> <img :src="item.thumbnail" style="width: 30px;height: 30px" alt=""/></span>
          <span style="line-height:30px;width: 220px;display:inline-block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"> <a style="" :href="'/DetailArticle?aid=' + item.id" target="_blank">

          {{item.title}}</a></span>
          <span style="color: grey;float:right;display: inline-block;line-height: 30px">{{ item.viewCount }}阅读</span>
...
export default {
  ...
  methods: {
    ...
    getHotArticleList() {
      hotArticleList(this.queryParams,false).then((response) => {

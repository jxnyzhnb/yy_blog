...
<template>
  <div class="detailBox tcommonBox">
    ...
    <header>
      ...
      <div style="border:1px solid #F7F7F7;height: 80px;background-color:#F7F7F7;border-radius: 5px ">
        <div style="margin-top: 7px;text-align: center"> <span style="font-weight: bold">{{ detailObj.nickName }}&nbsp;&nbsp;&nbsp;&nbsp;
                      <i class="fa fa-clock-o" aria-hidden="true">
                      </i></span>发表于 <span>{{ detailObj.createTime }}</span> •
          <i class="fa fa-fw fa-eye"></i>{{ detailObj.viewCount }} 次围观 •
        </div>
        <div style="margin-top: 8px;margin-left: 125px;height: 50px">
          文章标签:&nbsp;&nbsp;&nbsp;&nbsp;
          <span class="tag" v-for="(tag,index) in detailObj.tags" :key="'tag'+index">
                        <a :href="'/Home?q='+tag" style="color:#0066ff;" target="_blank">{{ tag }}</a>
                      </span>
        </div>
      </div>
      <div class="ui label">
    ...
    <div class="donate">
      <div class="donate-word" >
        <span @click="getLike" style="display: block" >
             <i v-if="!isLike" class="fa fa-thumbs-o-up" aria-hidden="true"><span  v-if="detailObj.likeCount!=0">{{ detailObj.likeCount }}</span></i>
          <i v-if="isLike" class="fa fa-thumbs-up" aria-hidden="true"><span v-if="detailObj.likeCount!=0">{{ detailObj.likeCount }}</span></i>
        </span>
      </div>
    </div>
      </div>
...
export default {
  ...
  methods: { //事件处理器
    ...
    getIsLike(){
      if (this.haslogin){
        getIsLike(this.aid,0).then(resposne=>{
    ...
    getLike() {
      ...
      if (this.haslogin) {
        ...
        if (this.isLike) {
          ...
          unLike(this.detailObj.id, 0)
          this.isLike = false
        } else {
          // setLike(id)
          this.detailObj.likeCount += 1
          setLike(this.detailObj.id, 0)
          this.isLike = true
        }
      } else {
        this.tip()
    ...
    getArticleDetail: function () {
      getArticle(this.aid,this.haslogin).then((response) => {
        this.detailObj = response
        const markdownIt = mavonEditor.getMarkdownIt()
        // markdownIt.re
        this.detailObj.content = markdownIt.render(response.content);
        console.log("this.detailObj.thumbnail"+this.detailObj.thumbnail)
        this.$emit("thumbnail",this.detailObj.thumbnail)
      })
    },
    ...
    routeChange: function () {
      ...
      this.$nextTick(function (){
        this.bus.$emit('articleId', this.aid)
      })

      console.log("dsds"+that.$route.query.aid)
      //判断用户是否存在

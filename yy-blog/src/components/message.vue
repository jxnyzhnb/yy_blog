...
<template>
  <div class="tmsgBox" ref="tmsgBox">
    <div class="tmsg-respond" ref="respondBox">
      <h3><small v-show="isRespond" class="tcolorm" @click="removeRespond">取消回复</small></h3>
      <form class="">
        <el-input
          ref="commentInput"
          type="textarea"
            ...
            <article class="">
              <header>
                ...
                <div class="i-time">
                  ...
                  <span style="font-size: 15px;margin-left: 10px;" @click="getLike(item)">
                                    <i v-if="!item.isLike" class="fa fa-thumbs-o-up" aria-hidden="true"><span
                                      v-if="item.likeCount!=0">{{ item.likeCount }}</span></i>
                                    <i v-if="item.isLike" style="color: #50bfff" class="fa fa-thumbs-up"
                ...
                <article class="">
                  <header>
                    ...
                    <div class="i-time">
                      ...
                      <span style="font-size: 15px;margin-left: 10px;" @click="getLike(citem)">
                        <i v-if="!citem.isLike" class="fa fa-thumbs-o-up" aria-hidden="true"><span
                          v-if="citem.likeCount!=0">{{ citem.likeCount }}</span></i>
                        <i v-if="citem.isLike" style="color: #50bfff" class="fa fa-thumbs-up likeComment"
                           aria-hidden="true"><span v-if="citem.likeCount!=0">{{ citem.likeCount }}</span></i>
                      </span>
                    </div>
                  </header>
                  <section>
                    <p v-html="analyzeEmoji(citem.content)">{{ citem.content }}</p>
                    <div v-show="haslogin" style="float:left;" class="tmsg-replay"
...
import {setLike, unLike} from "../api/like";
  ...
  methods: {
    ...
    getLike(comment) {
      ...
      if (this.haslogin) {
        ...
        if (comment.isLike) {
          // unLike(id)
          comment.likeCount -= 1
          unLike(comment.id, type)
          comment.isLike = false
        } else {
          // setLike(id)
          comment.likeCount += 1
          setLike(comment.id, type)
          comment.isLike = true
        }
      } else {
        this.tip()
    ...
    sendMsg: function () {//留言
      ...
      if (this.haslogin) {
        if (that.textarea) {
          that.sendTip = '咻~~';
          if (that.$route.path == '/DiscussDetail') {
            that.type = 1
            that.aid = that.did
          }
    ...
    showCommentList: function (initData) {//评论列表
      ...
      if (that.$route.name == 'DetailArticle') {//文章列表的评论
        ...
        that.queryParams.typeId=that.aid
        that.queryParams.type = 0;
      } else {//其他评论
        if (that.$route.name == 'DiscussDetail') {
          that.did = that.$route.query.did == undefined ? 1 : parseInt(that.$route.query.did);//获取传参的aid
          that.queryParams.typeId=that.did
          that.queryParams.type = 1
        }
      }
      getComment(that.queryParams, that.haslogin).then((response) => {

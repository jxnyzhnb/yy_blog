...
<template>
  <div class="">
        ...
        <el-col :span="24">
          ...
          <div class="headBox">
            <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect"
                     :router="true">
              <el-menu-item index="/Home" class="head1"><i class="fa fa-wa fa-home"></i> 首页</el-menu-item>
              <el-menu-item index="/discuss" class="head1"><i class="fa fa-square" aria-hidden="true"></i> 讨论区</el-menu-item>
              <el-menu-item v-show="haslogin" index="/MyArticle"><i class="fa fa-book" aria-hidden="true"></i> 文章
              </el-menu-item>
              <!--              <el-submenu index="/Share">
                              <template slot="title"><i class="fa fa-wa fa-archive"></i> 分类</template>
                              <el-menu-item style="color:black;background-color:white;width: 30px!important;  "
                                            v-for="(item,index) in classListObj" :key="'class1'+index"
                                            :index="'/Share?classId='+item.id">{{ item.name }}
                              </el-menu-item>
                            </el-submenu>-->
              <el-menu-item index="/Friendslink"><i class="fa fa-wa fa-users"></i>友链</el-menu-item>
              <el-menu-item v-show="haslogin" class="haslogin" index="/AddArticle"><i class="fa fa-pencil-square-o"
                                                                                      aria-hidden="true"></i> 创作
              </el-menu-item>
              <el-menu-item v-show="haslogin" class="haslogin" index="/MessageList">
                <el-badge :is-dot="isDot" class="item" style="height: 30px;line-height: 30px">
                  <i class="fa fa-bell" aria-hidden="true"></i>消息
                </el-badge>
              </el-menu-item>
              <el-menu-item index="/Messboard"><i class="fa fa-comment" aria-hidden="true"></i> 留言</el-menu-item>
              <div class="search">
                <el-input style="float:left;width: 200px" v-model="input" @keyup.enter.native="getSearch"
                          placeholder="请输入内容"></el-input>
                <el-button type="info" @click="getSearch"
                           style="float:left;margin-left:20px;line-height: 10px;margin-top: 3px" size="small">搜索
                </el-button>
              </div>
              <div class="userInfo">
                ...
                <div v-show="haslogin" class="haslogin">
                  <img :src="$store.state.avatar?$store.state.avatar:$store.state.errorImg"
                       style="height: 30px;width: 30px;border-radius: 100%;transition: all .4s ease-in-out;-webkit-transition: all .4s ease-in-out;object-fit: cover;margin-top: 3px"/>
    ...
    <!--    <div class="headImgBox"
             :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':this.headBg}">-->
    <div class="headImgBox" v-if="this.$route.path!='/Messboard'"
         :style="{backgroundImage:back?back:headBg}">
      <div class="scene">
        <div><span id="luke"></span></div>
      </div>
      <div class="h-information" v-if="this.haslogin">

        <img :src="$store.state.avatar?$store.state.avatar:$store.state.errorImg" alt="">

        <h2 class="h-description">

          <span style="font-weight: 500">个性签名:</span>{{ $store.state.signature ? $store.state.signature : "暂无" }}
...
import {getIsRead} from "../api/notice";
...
export default {
  ...
  data() { //选项 / 数据
    return {
      ...
      // path:'',//当前打开页面的路径
      input: '', //input输入内容
      headBg: 'url(static/img/headback01.png)', //头部背景图
      headTou: '', //头像
      projectList: '', //项目列表
      isDot: false,
      avatar: '',
  ...
  methods: { //事件处理器
    ...
    getSearch() {
      this.$router.push("/Home?q=" + this.input)
      //将搜索框中的关键字传给articleList组件
      this.bus.$emit('is', this.input)
    },
    getIsRead() {
      getIsRead().then((response) => {
        this.isDot = response
      })
    },
    getCategoryList() {
    ...
    handleSelect(key, keyPath) { //pc菜单选择
      console.log(this.activeIndex, "dsadsds111");
  ...
  created() { //生命周期函数
    ...
    this.routeChange();
    //获取查询参数
    this.input = that.$route.query.q == undefined ? '' : that.$route.query.q;
  ...
  mounted() { //页面元素加载完成
    if (this.$route.path!="/Messboard"){
      var that = this;
      var nickName = that.userInfo.nickName
      var timer = setTimeout(function () {
        Typeit(nickName, "#luke"); //打字机效果
        clearTimeout(timer);
      }, 500);
    }

    this.bus.$on('isDot', res => {
      this.getIsRead()
    })
    this.bus.$on('newAvatar', res => {
      this.avatar = res
    })
  }
}

    ...
    <div class="container">
      ...
      <div class="">
        <div v-if="login==1" class="loginBox">
          ...
          <div class="otherLogin" style="background-color: white">
            <a href="javascript:void(0)"><i class="fa fa-fw fa-wechat"></i></a>
            <a href="javascript:void(0)"><i class="fa fa-fw fa-qq"></i></a>
            <a href="javascript:void(0)" @click="githubLogin"><i class="fa  fa-5x fa-github" aria-hidden="true"></i></a>
          </div>
        </div>
...
<script>
import {loginGithub, userLogin, userRegister} from '../api/user.js'
  ...
  methods: { //事件处理器
    ...
    githubLogin() {
      window.open(
        ...
        this.$store.state.GITHUB_CLIENT_ID +
        "&grant_type=authorization_code&redirect_uri=" +
        this.$store.state.GITHUB_REDIRECT_URL +
    ...
    gotoHome: function () {//用户登录
      ...
      userLogin(this.username, this.password).then((response) => {
        ...
        this.$store.commit("login",response.userInfo)
      /*  this.$store.state.avatar = response.userInfo.avatar
        this.$store.state.id = response.userInfo.id
        this.$store.state.signature = response.userInfo.signature
        this.$store.state.nickname = response.userInfo.nickName*/
    ...
    thirdLogin() {
      showLoading();//打开加载中
      loginGithub(this.$route.query.code).then(response => {
        hideLoading();//关闭加载中
        // 登录成功记录token和用户信息，登录失败给对应提示
        setToken(response.token)
        // 存储用户信息
